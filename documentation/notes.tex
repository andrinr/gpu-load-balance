...
We compute $d_{leftCell}$ as described in equation \ref{eq:d1} and store its results in a variable named $d^\prime$. 
...
\ref{fig:orb2} \&  \ref{fig:orb22} is the SPTDS after initialization: there is only one $cell_1$ which is root and $V_{cell_1}$ encompasses the entire domain which in this case is the rectangle ranging from 0 to 1 in both the x and y axis. $d_{cell_1}$ is equivalent to 3 and $N_{cell_1}$ is 7. 
