...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Master Layer schedule of a an improved GPU parallelized ORB}
	\label{fig:orbgpup}
\end{figure}
...
As soon as the problem size approaches a more realistic size, meaning closer to the number of particles which are to be expected on a single node, the speedup appears to converge to a factor of 6. This is a positive outcome, as it would be undesirable to have a degrading performance.

Finally measurements were made where the problem size remains equivalent, however the number of desired leaf cells $d$ of the constructed SPTDS are varied. 
...
\begin{figure}[H]
	...
	\caption{Speedup of GPU over CPU version for a problem size of $N = 2^{32}$}
	\label{fig:analy4}
\end{figure}
