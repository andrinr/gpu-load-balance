...
\subsubsection{Init and Finalize}
...
The operative is essentially a C++ implementation of the code shown in figure \ref{proc:orbmain}. Since recursive algorithms are harder to optimize for the compiler, we use a for loop instead.

The main task of the operative is to coordinate all services and interpret their results. 
...
The entire schedule of the ORB is depicted in figure \ref{fig:orbgpup}. In a first step we call the initialize service, where the necessary data is allocated on the CPU and the GPU, and the particles are loaded. Next we already enter the main loop from the ORB, which iterates as until we have built a SPTDS with the desired size. In this loop the count service is called, this lets us know which cell has how many particles summed over all the threads in the system. To prepare the data for the GPU transfer we make the temporary array, which is then sent to the GPU. 
Everything is now set to start counting the particles left of the initial cut and improve the cut until we found a nearly perfect cut. Finally split all particles which have currently been processed and partition the particle arrays on each thread. The tree building loop is then repeated or we exit the loop and call the finalize service to free the memory which was allocated in the init service.
...
\subsubsection{Code}
The actual kernel code is depicted in figure \ref{cuda:reduction}, the most important performance crucial details of the code are:
