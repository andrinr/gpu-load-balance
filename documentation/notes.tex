...
PkDGrav, as well as most state of the art simulation softwares, use a space partitioning binary tree data structure (SPBTDS) to group object by spatial proximity. The root of the SPTDS stores the volume information of the entire dataset, the volume  The mutlipole expansion is then applied to all groups and the fast multipole method increases the runtime from $O(N^2)$ to $O(N)$ where $N$ is the total number of particles. \cite{Stadel2001}
...
\subsubsection{Advantage of Cubic Bounding Volumes}\ref{sec:cubic}
...
Most if not all state of the art supercomputers are hybrid, meaning each node has a CPU as well as a GPU.\cite{TOP500} GPU accelerated code can increase the speed of many computations, where in most cases CUDA is used to fully leverage the hardware. PkDGrav uses the GPU for its force computations. 
...
Where ates of now in PkDGrav and all other state of the art N-Body simulation in the realm of astrophysical simulations, steps (1.) and (2.) are not leveraging GPU acceleration.
...
N \times 3$. Nevertheless this corresponds a fraction of the total data transfer: $\frac{3}{\lceil \log_2 d \rceil \times}$. In figure \ref{fig:transfer} its observable how after d reaches $d$ the number of bytes to be transferred are less 
...
Its not clear if the partition kernel itself is also faster or even slower than a CPU variant, as the problem is not an ideal case of a GPU program. Most implementation require many control statements and additional data storage as doing an in place partitioning on the GPU is not possible.
