...
\begin{titlepage}
	\begin{center}
		...
		\Huge
		\textbf{Orthogonal Recursive Bisection on the GPU for Accelerated Load Balancing in Large N-Body Simulations}
...
In this paper I established an upper limit for the speedup of a GPU ORB implementation over its fully parallelized CPU only counterpart. In order to do so, I proposed a runtime estimate for both versions. As hardware specific details and some knowledge about compilers and Assembly instructions are crucial for a reliable estimate, I explored the theoretical foundations of memory bandwidth and processing power. 
...
The aim of this paper is to improve the runtime of ORB. In terms load balancing, there is a distinction between workload and memory balancing. 
...
\begin{figure}[H]
	\begin{center}
		...
		\end{minipage}
	\end{center}
	\caption{Example 2D particle distribution with $N$ = 7}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Volumes of SPTDS leaf cells generated from ORB at a depth of 0}
	\label{fig:orb1}
\end{figure}
...
\begin{figure}[H]
	...
	\end{forest}    
	\caption{SPTDS generated from ORB at a recursion depth of 0}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Volumes of SPTDS leaf cells generated from ORB at a depth of 1}
	\label{fig:orb2}
\end{figure}
...
\begin{figure}[H]
	...
	\end{forest}    
	\caption{SPTDS generated from ORB at a recursion depth of 1}
	\label{fig:orb22}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Volumes of SPTDS leaf cells generated from ORB at a depth of 2}
	\label{fig:orb3}
\end{figure}
...
\begin{figure}[H]
	...
	\end{forest}
	\caption{SPTDS generated from ORB at a recursion depth of 2}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
\caption{Computing model}
\end{figure}

\subsection{Supercomputers}

For Piz Daint, Summit and Eiger all relevant hardware metrics were collected and compiled to table \ref{fig:datapoints}. Piz Daint and Eiger were chosen, because the systems can be used to test the code. Furthermore Summit is included as it is, at the time of writing this paper, one of the most capable supercomputers in the world.
...
\begin{figure}[H]
	\begin{center}
		\begin{tikzpicture}
				...
				\end{axis}
			\end{tikzpicture}
	\caption{Roofline Model for the isolated Count Left code on Piz Daint}
	\label{fig:roofdaint}
	\end{center}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		\begin{tikzpicture}
			...
			\end{axis}
		\end{tikzpicture}
		\caption{Roofline Model for the isolated Count Left code on Piz Daint using the GPU}
...
\begin{figure}[H]
	\begin{center}
		\begin{tikzpicture}
			...
			\end{axis}
		\end{tikzpicture}
		\caption{Roofline Model for the isolated Count Left code on Summit using the GPU}
...
\begin{figure}
	...
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Roofline Model for the isolated Count Left code on Alps}
	\label{fig:roofalps}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\end{figure}
	\end{center}
	\caption{Tree stored as an array heap}
	\label{fig:treeheap}
\end{figure}
...
\begin{figure}[H]
	...
	\end{forest}
	\caption{SPTDS constructed from example particle distribution tree with 3 leaf cells ($d$ = 3)}
	\label{fig:extree}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Master later schedule of a CPU only ORB implementation}
	\label{fig:orbp}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
\caption{Fundamental CUDA concepts}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Master later schedule of a GPU accelerated ORB implementation}
	\label{fig:orbgpup}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		\begin{tikzpicture}
			...
			\end{axis}
		\end{tikzpicture}
		\caption{Kernel calls with GPU Count Left with two different versions}
		\label{fig:degrading}
	\end{center}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Master later schedule of an improved GPU accelerated ORB implementation}
	\label{fig:orbgpup2}
\end{figure}
...
On lines 26 - 27 a warp level reduction is performed, which is equivalent to the one we have explained with the previous kernel. Now each tread with a $lane$ equivalent to 0 has stored the sum of 31 next consecutive threads in local memory. In the case of 256 threads this means that there are 8 elements left to be summed up, however this step cannot be performed on a warp level as all values are stored in distinct warps. Therefore $val$ is read from local memory and written to shared memory at an index of $warpId$ and a \textit{\_syncthreads()} call is made (line 29-32). In this case it is necessary to synchronize all threads, as the storing of elements happens across warps.
The number of values stored in shared memory can be determined by dividing the number of threads in a block by the warp size. Thus all threads for which a value in shared memory was written in the previous step, load the said value back to local memory, the others store 0 in their local memory (line 34-35). At this point a final warp level reduction can be performed (lines 34-41). A single element on thread 0 stores the entire sum across all input data elements and it can be written to global memory and later fetched from the CPU.
...
\begin{figure}[H]
	\begin{center}
		...
		\end{tikzpicture}
	\end{center}
	\caption{Master later schedule of a GPU accelerated ORB implementation with GPU partitioning}
	\label{fig:orbgpu3}
\end{figure}
...
\begin{figure}[H]
	\begin{center}
		\begin{tikzpicture}
			...
			\begin{axis}
				...
				\addplot+[ybar] plot coordinates {(cpu, 38.46) (gpu, 6.84)};
				\addplot+[ybar] plot coordinates {(cpu, 15.77) (gpu, 15.78)};
	...
	\caption{Runtimes for $N = 2^{30}$ and $d=4096$ on a single Piz Daint node}
	\label{fig:analy2}
\end{figure}

In figure \ref{fig:analy2} the final measurements are plotted for $N = 2^{30}$ and $d=4096$ on a single Piz Daint node. The reason why the measurements were not performed on multiple nodes, is that the paper does not propose any changes to the internode communication strategies, therefore these results are not of importance. A uniform random distribution was used instead of the target data, because at time of estimating the runtime there was no dataset available with the proper distribution. The results should however be comparable as solely the exact number of iterations taken to find a cut for each cell varies on different datasets.

A large portion of the total ORB runtime is attributed to the partition part. If the time lost during the partitioning is subtracted, the GPU version is about $5.6$ times faster than the CPU implementation, which is slightly worse than the expected speedup of $6.2$. The difference can be explained by the GPU reduction kernel which is slightly worse than the memory bandwidth would allow as described in section \ref{imprctlwrp}. The theoretical model on the other assumes an implementation which performs just as well as the memory bandwidth would allow. 

When comparing the absolute runtimes there is a stark difference visible: $67.77s$ were predicted for the CPU version where $38.46s$ was the actual timing. Similarly the GPU was expected to execute ORB\footnote{Excluding the time required for the partition} in $10.79s$, however it only took $6.84s$. The theoretical model considers an upper bound for the total number of iterations which the bisection method takes to converge. In reality this number is closer to 20 than to $p$ which is 32 in the input data. When considering this, the measurements are also in an absolute term relatively close to the theoretical model. $67.47s \times \frac{20}{32} = 42.16s$ against the actual $38.46s$ and $10.79s \times \frac{20}{32} = 6.74s$ compared to the measured $6.84s$.
Concluding the measured runtime are surprisingly similar to the expected ones, however they do not reach the theoretical maximum. The reduction kernel would need to be further improved to fully leverage the GPU bandwidth, however as of writing this paper no better implementation variant is known. 


\subsection{Scaling}

The CPU and GPU versions were also tested for different problem sizes $N$. The results were then compiled into speedups, enabling us to see how the speedup behaves for different problem sizes. 
...
\begin{filecontents*}{speedups.csv}
	p, d, s
	1048576 , 10, 1.107
	4194304, 10, 2.086
	16777216 , 10, 3.798
...
\begin{figure}[H]
	...
	\caption{Speedup of GPU over CPU version for different problem sizes and $d = 4096$}
	\label{fig:analy3}
\end{figure}

In the resulting figure \ref{fig:analy3} it can be observed how the speedup is only minor for \textit{small} problem sizes. This is most certainly caused by an overhead introduced by the GPU implementation over its CPU counterpart, as kernel invocations and memory copying operations are not free of cost. 
As soon as the problem size approaches a more realistic size, meaning closer to the number of particles which are to be expected on a single node, the speedup appears to converge to a factor of 6. This is a positive outcome, as it would be undesirable to have a degrading performance for larger problem sizes. 

\newpage
\section{Conclusion}

This research aimed at improving the runtime of the ORB algorithm, which constructs a space partitioning tree data structure, using the GPU programming language CUDA. The SPTDS is used for the FMM method as well as load balancing. 

In order to have an idea how large the speedup of an optimal GPU implementation against an optimal CPU version is, I have calculated runtime for both versions. In theory a maximal speedup of 6.2 was expected. 

With extensive research about the CUDA programming language I have written three GPU reduction kernels, where kernel2 significantly outperformed the first one. The kernel essentially solves a segmented map reduce problem. NVIDIA's CUB \cite{cubSeg} exposes a similar function, however in the case of ORB, the map condition varies across segments which is not supported in CUB. 
I have introduced another version, kernel3 which makes extensive use of warp level primitives and can reduce access to shared memory by a factor of 32, however it did not outperform kernel2 and measurements have shown that the throughput of kernel2 and kernel3 is close to GPU memory bandwidth.

With the gained knowledge I have tried to accelerate the partitioning using CUDA. However due to time constraints I was not able to finish writing the required kernels. Thus I described the theoretical concepts, which can build a solid foundation for further research. However it is not clear wether the GPU partitioning could further improve the ORB runtime when implemented.
Finally I have made time measurements on a single node of the Piz Daint supercomputer. When isolating the time it takes to compute a cut, without considering the time used for partitioning, the GPU accelerated version I have implemented runs about 5.6 times faster than a fully optimized and parallelized CPU version. Which comes close to the theoretical limit and the initial goal of improving the runtime of ORB is successful. 

The section of ORB which could still be improved by a lot, is the choice of the bisection method. As of now many iterations are needed to find a ideal cut. In theory more sophisticated root finding methods or binning algorithms\cite{rrt} could be employed, further improving the runtime of ORB. 
