...
Lets us now consider Piz Daint. We can plug in the values which we have determined into the formula, where we do so for $np = 1, 2$ and $3$.

\begin{center}
	\begin{equation}
		121.6 gflops = 3.8 * 8 * \frac{1}{0.5} * 2
	\end{equation}
	\label{eq:daintp1}
\end{center}
...
\begin{center}
	\begin{equation}
		243.2 gflops = 3.8 * 8 * \frac{1}{0.5} * 4
	\end{equation}
	\label{eq:daintp2}
\end{center}
...
\begin{center}
	\begin{equation}
		486.4 gflops = 3.8 * 8 * \frac{1}{0.5} * 8
	\end{equation}
	\label{eq:daintp3}
\end{center}

We plot those numbers as GFLOPS peak values for Piz Daint, we add a line depicting the memory bandwidth which can 

\TODO{Explain connection from formula in the context of graphs}
\begin{figure}[H]
	\begin{center}
		\begin{tikzpicture}
				\begin{axis}[
...
In this section describe the stand-alone implementation of the accelerated ORB algorithm. We will develop this somewhat separately from the PKDGrav codebase in order to have smaller and easily understandable codebase. We will however use mdl2 from PKDGrav, which is used to distribute the workload among the processors in the system. In section \ref{sec:gpuimpl} we describe the GPU implementations. The CPU and GPU versions are merged in a single program, where different acceleration strategies can be selected and evaluated.

We will refrain from describing each part of the code in detail, however any performance critical details and all the important data-structures are described. Furthermore, we will pay special attention to the CUDA kernels and also memory management strategies, first of all since only minor differences in coding style can have a great impact, because the kernel are the most crucial parts of the optimization and finally because it increases replicability of the code.
...
\vspace{0.5cm}
We can derive the following constraints given the number of leaf cells $d$. The constraints are helpful for many parts of the code.
...
If we apply the formulas to the example tree we can observe that the depth indeed corresponds to: $\lceil log_2(d) \rceil = 2$. The number of leaves on the second last level are  $2^{2} - 3 = 1$ and the number of leaves on the last level are $2 \times 3 - 2^{2} = 2$. Finally the total number of cells is $2\times 3 - 1 = 5$.

\subsubsection{Class}
The cell class consists of the following variables:

\begin{itemize}
	\item $id$: The id is a unique identification of the cell instance and corresponds to the position in the heap data structure plus one. It useful to directly compute the indices of both child cells using the above shown formulas.
	\item $nLeafCells$: this variable is equal to $d_{cell}$ and depicts the number of leaf cells which can be found when walking all possible child cells of this cell. This variable is important when building the tree as we need to keep track how often the cell has to be split. 
...
The cell class has some helpful helper functions. \TODO{extend}

\subsection{Operative}

\TODO{Check if loop is really better when iterative. Or maybe dont argue too much}
Since the recursive version as shown in \ref{proc:orb} is not very compiler friendly, meaning its hard to properly optimize it at compile time, we introduce another non recursive version.

\TODO{Write here in pseudcode as C++ version to long}
\begin{figure}
	
	\begin{algorithm}[H]
		\caption{Unrolled ORB main routine}\label{euclid}
		\begin{algorithmic}[1]
			\Procedure{orb}{$\vec{lower}, \vec{upper}, d$}
			\State $\vec{size} = \vec{upper} - \vec{lower}$
			\State $i = maxIndex(\vec{size}_0, \vec{size}_1, \vec{size}_2)$ \Comment{Get index of max value}
...
\newpage
\section{GPU Implementations}\label{sec:gpuimpl}
...
Each kernel is executed exactly once for every thread and has private local memory, only accessible by the thread. Furthermore each kernel is part of a block, where a typical kernel is usually called with hundreds to thousands of blocks. Each block has access to shared memory register, where the capacity of this register can be defined at runtime by the host. The maximum shared memory size depends on the hardware but usually there are 48kB per block available. There are also upper limits for the number of threads per block, usually around 512 to 1024. With a shared memory size of 48kb and 1024 threads we get $\frac{48000b}{1024} = 46b$ per thread. This is equivalent to a little bit more than one float per thread. However usually we do not max out the thread counts per block, leaving us with more shared memory space per thread. 

Data in the shared memory exists only as long as the kernel exists and it cannot be accessed across different blocks. Race conditions do apply to shared memory, but threads in a block can be synchronized, thus enabling us to ensure a deterministic result. 

Additionally to the shared memory, there exists global memory. Global memory is persistent even after a block or a kernel has finished, allowing for data to be copied from the host to device, and reused over a long time period and across different blocks. Global memory is however significantly slower than shared memory and it is best practice to copy data from global memory to shared memory before performing actual computations on it. After the computations have finished, the results can be written back to global memory.

Whilst threads within a block can be synchronized, this is not the case for the blocks launched from a kernel. The only real synchronization technique, is to wait for a kernel, meaning all blocks associated with this kernel, to finish and execute a consecutive task using another kernel. The number of blocks and the number of threads per block can be defined dynamically during runtime as seen in figure \ref{cuda:call}.

Since the GPU is limited in its computing and also memory capabilities, the number of blocks which are run in parallel are limited, therefore CUDA cannot give us any guarantee, weather a set of blocks are run serially or in parallel. Some of the blocks might be run in parallel, meanwhile other are run serially. This very constraint, forces the programmer to rethink algorithms and think of ways, how individual subproblems can be run somewhat independently of each other. 
