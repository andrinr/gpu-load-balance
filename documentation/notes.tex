...
\subsection{GPU Accelerated Count Left With Atomics}

To solve the degrading performance problem we make some changes to the kernel and the overall schedule. The main idea is to prepare all the necessary data for each of the running blocks, meaning each block knows over which slice of the particles data is has to iterate, furthermore it knows the cut position, which can vary depending on which cell the particles belong to.
Using the atomic add on a global index variable, we can make sure that each 
