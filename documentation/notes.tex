...
We have sucessfully implemented a version with a GPU accelerated Count Left. Since the particle is still partitioned on the CPU, the particle data needs to be copied to GPU each time we expand the SPTDS by another level. As we use the Make Axis service to compress the axes into a single array, we only need to copy $N$ elements, but we need to do so $\lceil \log_2 d \rceil$ times. 
If we partition the particles on the GPU, the data needs to be copied to the GPU only once, but in this case all three coordinates need to be copied, thus $
N \times 3$. Nevertheless this corresponds a fractional of the total data transfer: $\frac{3}{\lceil \log_2 d \rceil \times}$. 
