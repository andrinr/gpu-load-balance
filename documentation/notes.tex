...
The partitioning of the particles array comes with two advantages:

\begin{enumerate}
	\item We can easily keep track of the particles contained within one cell by simply storing the start and end index of the elements within the volume. 
	\item We can easily perform operations on the particle data contained within a cell since its stored in a consecutive array. 
\end{enumerate}
...
Initially we assume that each processor has a random unique subset of all the particles stored in its memory. If we apply this concept to the particles from our numeric example, we have a distribution as depicted below. 
...
For modern hardware, its fairly uncommon to release a peak flops value. Since 2008 Intel has introduced AVX (Advanced Vector Extensions), gflops benchmarks largely depend upon the compiled assembly instructions, the specific instructions vary a lot between small c++ implementation details and can only be used on contiguous memory access. Therefore we estime the glfops with our application specific context in mind. 
