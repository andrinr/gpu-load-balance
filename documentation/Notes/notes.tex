...
\begin{itemize}
	\item GPU Accelerated Count Left
	\item GPU Accelerated Count Left and Partition
\end{itemize}

Note that the total number of particles, which can be simulated, will be reduced, as the GPU memory is a lot smaller than the CPU memory. How much less particles can be simulated, depends a lot on the implementation and will vary between the two implementations.
...
\begin{itemize}
	...
	\item \textbf{Shared Memory Bank Conflicts} In order to decrease the bank conflicts in the shared memory we want to make sure that threads in the same warp do not try to access the same memory bank. Therefore we offset each memory access by the total gridSize as it can be seen on line 12 in figure \ref{cuda:reduction}
	
	\item \textbf{Coalesced Memory Access}
	\TODO{Do}
