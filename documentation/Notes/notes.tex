...
We can back this claim by comparing tflops with 
	...
	\begin{center}
		\begin{tikzpicture}
			\begin{axis}[
				...
				xlabel={\# Processors},
				ylabel={tflops},
				xmin = 1, xmax = 16, ymin= -0.02, ymax=0.3,
				legend style={at={(0.6,0.3)},anchor=west}]
				
				\addplot+[name path = A, domain = 1:16, mark=none] {
					4.5 * 8 * 2* \x / 1000
				};	
	
				\addlegendentryexpanded{4.5 GHZ, $100\%$ parallel}
				
				\addplot+[name path = B, domain = 1:16, mark=none] {
					2.5 * 8 * 2 * (0.8 * \x + 0.2) / 1000
				};	
			
	
				\addlegendentryexpanded{2.5 GHZ, $80\%$ parallel}
