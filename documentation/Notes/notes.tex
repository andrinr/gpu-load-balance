...
\subsection{Optimized Memory Layout}

Describe how we swap sections of the particle array in order to reduce 
\csvreader[
tabular=ccc,
table head=\hline \bfseries{x} & \bfseries{y} & \bfseries{id} \\\hline,
late after last line=\\\hline % horizontal line at the end of the table
...
table head=\hline \bfseries{x} & \bfseries{y} & \bfseries{id} \\\hline,
late after last line=\\\hline % horizontal line at the end of the table
]{particles10.csv}{}{\csvcoli & \csvcolvi & \csvcolvii}
...
\TODO{If there is time, build simple MPI version to backup model with data.}

\TODO{Try to integrate: https://zsmith.co/bandwidth.php}
	...
	\begin{center}
		\begin{tikzpicture}
			\begin{axis}[
				...
				xlabel={\# Processors},
				ylabel={tflops},
				xmin = 0.5, xmax = 16, ymin= -0.1, ymax=0.6,
				legend style={at={(0.03,0.8)},anchor=west}]
				
				\addplot+[domain = 1:16, mark=none] {
					4.5 * 8 * \x / 1000
				};	
				...
				\addplot+[mark=none] coordinates {(0,0.002) (16,0.002)};
				\addlegendentry{$B_{C}$ Intel}
				
				\addplot+[mark=none] coordinates {(0,0.005) (16,0.005)};
				\addlegendentry{$B_{C}$ IBM}
				
				\addplot+[mark=none] coordinates {(0,0.0062) (16,0.0062)};
...
Finish this argumentation and explain why we only consider memory access times as they far outweigh tflops. 
...
Blitz++ allows us to easily access the C-style array with the command $array.data()$, enabling us to switch to C-style arrays, whenever its deemed necessarily. We have observed cases where iterating over a blitz array does not enable AVX, however iterating over the underlying C-style array did enable AVX.


\subsection{Cell}

As our tree consists of multiple cells, which are to be stored in a tree data-structure. As seen in section \ref{section:orb-example} the conditions for a nearly complete binary tree hold, allowing us to use a heap to store the tree. This \TODO{hint} reduces the runtime of the tree, and since we need to store the cells in an array for communication purposes anyways (\TODO{Why exactly?}), this makes a lot of sense.

\TODO{Either copy cell into buffer. You have to linearize. Add arguments with leaf cells.}

When using a parallelization schema using local reshuffling we will have to send all cells on a single level using the messaging interface. Using the heap datastructure, cells which corresponds to single level in the tree, can be accessed using a simple range.
...
The cells could be stored as binary tree or as we have already mentioned we could make use of a heap data-structure which allows for greater flexibility. Due to performance improvements, we will opt for the second variant.


\subsection{Services}

As required by mdl, we make use of services, which essentially is a collection of functions which can be sent to any set of processors in the computing system. We abstract every performance critical part of the orb main runtime, which can parallelized into such a service.
...
\begin{itemize}
	...
	\item $countService$ : Counts the number particles inside the domain for each of the provided cells.
...
\subsubsection{CUDA Reductions}

Reductions in CUDA are a common explored application of kernels and we were able to reuse highly optimized kernels, proposed in a CUDA webinar as the basis for the counting algorithms, which is part of the binary cut algorithm. The only thing which needs to change, is that we don`t simply sum up the values in an array, but we count the number of elements which are smaller than the proposed cut value. Other than that we can essentially copy the mentioned reduction code.

\subsubsection{Send to device}

In order to prepare the data and send it to a device... 

\subsubsection{CUDA streams}

We will leverage CUDA streams to send data from the host to the device in parallel. With streams the CUDA Kernel engine can already start processing data, when the first batch, in this case the particles owned by processor number 0 are transferred to the GPU. 
