	...
	\begin{center}
		\begin{tikzpicture}
			\begin{axis}[
				...
				\addlegendentryexpanded{3.8 GHZ \& np=1};
	...
	\begin{center}
		\begin{tikzpicture}
			\begin{axis}[
				...
				\addlegendentryexpanded{4 GHZ \& np=1};
	...
	\begin{center}
		\begin{tikzpicture}
			\begin{axis}[
				...
				\addlegendentryexpanded{3.4 GHZ \& np=1};
...
\end{figure}
\end{comment}
Another important concept are warps. Each warp consists of 32 threads and all thread within a warp are executed in parallel, given that there is no warp divergence. Because warps are executed in parallel, there is no need for synchronization. A warp divergence can occur, if we have control statements within a warp, where two or more threads from the same warp execute a different code. This leads to a decrease in performance and should be avoided whenever possible, this can be ensured by letting each consecutive group of 32 threads execute the same code. 
