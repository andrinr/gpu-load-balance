...
\begin{figure}[H]
	\begin{center}
		\begin{tikzpicture}
			...
			\draw[red, very thick, stealth-stealth] (cpum1) to (cpu1);
			\draw[red, very thick, stealth-stealth] (cpum2) to (cpu2);
			\draw[red, double, very thick, stealth-stealth] (cpu1) to (cpu2);
			
			
			\draw[orange, dashed, very thick, stealth-stealth] (cpu1) to (gpu1);
			\draw[orange, dashed, very thick, stealth-stealth] (cpu1) to (gpu2);
			\draw[orange, dashed, very thick, stealth-stealth] (cpu2) to (gpu3);
			\draw[orange, dashed, very thick, stealth-stealth] (cpu2) to (gpu4);
			
			\draw[cyan, very thick, stealth-stealth] (gpum1) to (gpu1);
			\draw[cyan, very thick, stealth-stealth] (gpum2) to (gpu2);
			\draw[cyan, very thick, stealth-stealth] (gpum3) to (gpu3);
			\draw[cyan, very thick, stealth-stealth] (gpum4) to (gpu4);
			
			\draw[cyan, double, very thick, stealth-stealth] (gpu1) to (gpu2);
			\draw[cyan, double, very thick, stealth-stealth] (gpu3) to (gpu4);
			
			
			\node(al) at (-2cm, -3.5cm) {};
			\node (ar) at (-3cm, -3.5cm) {};
			\node[align=left] () at (0.5cm, -3.5cm) {CPU Mem Bandwidth, $B_{C}$};
			\draw[red, very thick, stealth-stealth] (al) to (ar);
			
			\node(al) at (-2cm, -4cm) {};
			\node (ar) at (-3cm, -4cm) {};
			\node[align=left] () at (0.5cm, -4cm) {GPU Mem Bandwidth, $B_{G}$};
			\draw[cyan, very thick, stealth-stealth] (al) to (ar);
			
			\node (bl) at (-2cm, -4.5cm) {};
			\node (br) at (-3cm, -4.5cm) {};
			\node[align=left] () at (0.5cm, -4.5cm) { CPU-CPU Interconnect, $I_{CC}$};
			\draw[red, double, very thick, stealth-stealth] (bl) to (br);
			
			\node (cl) at (-2cm, -5cm) {};
			\node (cr) at (-3cm, -5cm) {};
			\node[align=left] () at (0.5cm, -5cm) { GPU-CPU Interconnect, $I_{GC}$};
			\draw[orange, dashed, very thick, stealth-stealth] (cl) to (cr);
			
			\node (dl) at (-2cm, -5.5cm) {};
			\node (dr) at (-3cm, -5.5cm) {};
			\node[align=left] () at (0.5cm, -5.5cm) { GPU-GPU Interconnect, $I_{GG}$};
			\draw[cyan, double, very thick, stealth-stealth] (dl) to (dr);
...
\begin{center}
	\begin{equation}
		max\{ log(d) \times \left ( p \times \frac{ s }{B_{C}} \right ) = t
		\label{eq:cpu}, GHZ_{C} / Cores_{C} * N \}
	\end{equation}
\end{center}
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[xmin = -1, xmax = 13, ymin=-1, ymax=40]
\addplot[domain = 0:12,blue] {
	ln(\d) / ln(2) * \p * 
		max(x / (900 * 6), x/(4*22))
	+  x / (50 * 6) + x /(170 * 2) 
};
