...
The general idea of the ORB algorithm is to split a $k$ dimensional domain containing $N$ particles into $s$ subdomains. Since we are using ORB to make partitions, the subdomain sizes should be chosen such that the workload can be equally distributed among nodes / processors. Thus we want each domain to contain a similar number of particles. 

The easiest way to accomplish such a task, is search for a split such that 50\% of the particles are on the left side and likewise on the right side of the split. This split can be found in $O(log(N))$ using a binary search. We can introduce an even tighter bound when assuming each particle is being stored as a 32 bit precision number. In this case we have reached the maximum precision after $O(32)$ iterations. After such a split has been found, we can create two new subdomains and recursively apply the algorithm until we have reached the desired number of subdomains. To a clearer terminology, we will from now on refer to a domain as a cell. Furthermore a subdomain of a domain is a child cell of a cell, and likewise we also have the concept of a parent cell.

When each cell keeps track of its child cells, we have built a tree datastructure, where each node represents a cell, the root node is the cell which encompasses the entire space and particles, and finally its leaves can be assigned to a processor / node. Note that the tree is not balanced, but each node has either two children or is a leaf. 
