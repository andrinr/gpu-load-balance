...
As our tree consists of multiple cells, which are to be stored in a tree data-structure, but we have seen that the conditions for a balanced tree must always hold, we can use a heap, and thus a simple array to store all cells.
...
Note that due to the property of always assigning the larger number of leaf cells to the left child cell, there will never be gaps in the lowest level of the tree, i.e. all consecutive indices in the array are filled with a cell until the last element.
...
Furthermore the following constraints can be derived:

\begin{itemize}
	\item Given $d$ leaf cells, we can compute the depth of the tree with: $\lceil log(d) \rceil$
	\item Given $d$ leaf cells, the total number of cells are:
	$\lfloor log(d) \rfloor$ + 
\end{itemize}
