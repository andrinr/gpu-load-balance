...
We implement the following core classes and functions:

\begin{itemize}
	\item \textbf{ORB}: The orb class keeps track of 
\end{itemize}


\subsection{Architecture}

The stand alone version of the ORB should have following properties:

\begin{itemize}
	\item Work with MPI for CPU parallelization
	\item Work with CUDA for GPU parallelization
	\item Abstractions which allow profiling of different strategies such as local and global reshuffling
\end{itemize}
...
\begin{itemize}
	\item $countLeftService$ : Counts the number of particles left of the cut position for each of the provided cells.
	...
	\item $buildTreeService$ 
\end{itemize}

With this system, a processor simply needs to wait for new services and fetch their corresponding parameters. The data can be distributed to the processors with any preferred communication method, in our case we will use MPI for this purpose, but the abstraction allows to easily interchange the communicator.
