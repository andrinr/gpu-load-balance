...
\begin{itemize}
	...
	\item We define the space of binary numbers with a precision of $p$ as $\mathbb{B}_p = \{0,1\}^{p}$
	\item We define the size of the spatial domain with $\vec{b}$ where we have $\vec{b} \in \mathbb{B}_p^3$. 
	\item We define the coordinates of a particle $p_i$ as $\vec{x_i}$ for which holds $\{\vec{x} | -\frac{\vec{b}}{2} < \vec{x} < \frac{\vec{b}}{2}, \vec{x} \in \mathbb{B}_p^3 \}$.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		\Procedure{orb}{$\vec{lower}, \vec{upper} ,x, d$}
...
The exact runtimes of the ORB can only be estimated when considering specific hardware components. In this section we will establish formulas for different possible variants, which can be used to leverage GPU acceleration. We will compare these with the multiprocessor CPU variant and finally draw a conclusion, which of the variants are worth implementing. 

In order to have a clear terminology and understanding of a computing system, we will first describe the computing model of most modern high performance systems.
...
\begin{figure}[H]
	...
	\end{center}
\end{figure}
Furthermore we make the following assumption about the data:

\begin{itemize}
	\item
	$p = 32$ thus a single particle requires $3 \times 32bits = 96bits$ which is equivalent to $12Bytes$
	
	\item 
	All particles are stored on the CPU memory initially.
	
	\item
	We analyze one billipon particles, which translates to $10^9 \times  12 Bytes = 12 Gigabytes$. We refere to the size of the data as $s$.
